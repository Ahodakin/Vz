...
@section("title")
www.monassurance.ci :: Votre Dévis Voyage
@endsection
...
<script>
  ...
  function search_profile(id){

      $.get("/rest-api/v1/searchuser/"+id, function(data) {
          if(data!=0){
           ...
           $("#nationality").val(d.nationality_id);
             $('#nationality')
              .append($("<option/>") //add option tag in select
                  .val(d.nationality_id) //set value for option to post it
                  .text("{{ getCountryById("+d.nationality_id+") }}")) //set a text for show in select
              .val(d.nationality_id) //select option of select2
              .trigger("change"); //apply to select2

            $('#num_passport').val(d.passport_num)

            $('#expire_passport').val(d.date_expire_passport.split('-')[2]+'/'+d.date_expire_passport.split('-')[1]+'/'+d.date_expire_passport.split('-')[0])

           $('#modal-selectprofile').modal('hide')
            sfw.next()
          }
          else{
...
<div class="modal fade" id="modal-selectprofile" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Choississez un profile existant </h5>
      ...
      <div class="modal-body" id="selectcar_div">
        <section>
        @foreach($travel_profiles as $k=>$p)
        <div class="col-md-4">
          <input type="radio" id="control_{{$k}}" name="select" value="{{$p->id}}" >
          <label for="control_{{$k}}" class="label_radio_box" data-user="{{ $p->id }}">
            <h2>{{ $p->lastname }} {{ $p->firstname }}</h2>
            <p>Sexe : {{ ($p->gender=="H")?'Homme':'Femme' }}</p>
            <p>Date de naissance : {{ date("d/m/Y", strtotime($p->dob)) }}</p>
            @if(!is_numeric( intval(substr($p->email, 0,1))))
              {{$p->email}}
            @endif
            <p>N° Telephone : {{ $p->contact }} </p>
            <hr/>
            <p>Nationnalité : {{ getCountryById($p->nationality_id) }}</p>
            <p>N° Passeport : {{ $p->passport_num }}</p>
            <p>Date d'expiration : {{ date("d/m/Y", strtotime($p->date_expire_passport)) }}</p>
          </label>
        </div>
        @endforeach
        </section>
				...
				<form class="form-horizontal form-validation" id="quoteFormVoyage" method="post">
                    ...
                    <input type="hidden" name="_form_type_" id="_token" value="{{encrypt('VOYAGE')}}">
                             ...
                             <div class="form-group">
                               <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                  ...
                                  <input readonly data-parsley-group="block0" value="{{ Auth::guard('space_perso')->user()->phone_number }}" placeholder="Ex: 01020304" type="text" class="form-control" required name="phone_souscr" id="phone_souscr">
                        ...
                        <fieldset>
                            ...
                            <div class="row">
                              <div class="col-md-12 text-center">
                                <a href="#" data-toggle="modal" data-target="#modal-selectprofile" class="btn btn-primary get-in-touch" data-text="Utiliser un profil existant"><i class="fa fa-user"></i>Utiliser un profil existant</a>
                              </div>
                            </div>
                            ...
                            <div class="form-group">
                              ...
                              <div class="col-md-6">
                                ...
                                <div class='input-group datetimepicker_dob'>
                                  <input type="text" class="form-control dob" required data-parsley-group="block1" name="dob" id="dob" placeholder="JJ/MM/AAAA">
                            ...
                            <div class="form-group">
                              <div class="col-md-6">
                                ...
                                <input placeholder="Ex: votreemail@email.com"  type="email" class="form-control" name="email" id="email">
                              </div>
                               <div class="col-md-6">
